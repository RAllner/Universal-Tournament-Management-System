...
class TournamentController extends FrontendController
{
    ...
    public function dummyAdd()
    {
        $match = new TournamentMatch();
        $match->matchID = (string)21234;
        $match->stage = 1;
        $match->round = 5;
        $match->follow_winner_and_loser_match_ids = (string)123;
        ...
        foreach ($openMatches as $match) {
            ...
            if(!is_null($match->seed_A) && !is_null($match->seed_B)){
                $participant_A = Participant::find()
                    ...
                    ->one();
                /** @var Participant participant_id_A */
                $participant_B = Participant::find()
                    ->where(['tournament_id' => $model->id])
                    ->andWhere(['seed' => $match->seed_B])
                    ->one();
                /** @var Participant participant_id_B */
    ...
    public
    function calculateTournamentProgress()
    {

    }

    public
    function calculateGroupStageProgress()
    {

    }

    public
    function calculateFinalStageProgress()
    {
